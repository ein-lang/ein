...
impl FileStorage {
    ...
    pub fn resolve_absolute_file_path(&self, file_path: &app::FilePath) -> std::path::PathBuf {
        self.directory
            .join(self.resolve_relative_file_path(file_path))
    }

    pub fn resolve_relative_file_path(&self, file_path: &app::FilePath) -> std::path::PathBuf {
        file_path
            .components()
            .iter()
...
impl app::FileStorage for FileStorage {
    fn exists(&self, file_path: &app::FilePath) -> bool {
        self.resolve_absolute_file_path(file_path).exists()
    }
    ...
    fn read_to_vec(&self, file_path: &app::FilePath) -> Result<Vec<u8>, std::io::Error> {
        std::fs::read(self.resolve_absolute_file_path(file_path))
    }

    fn read_to_string(&self, file_path: &app::FilePath) -> Result<String, std::io::Error> {
        std::fs::read_to_string(self.resolve_absolute_file_path(file_path))
    }

    fn write(&self, file_path: &app::FilePath, data: &[u8]) -> Result<(), std::io::Error> {
        let path = self.resolve_absolute_file_path(file_path);
